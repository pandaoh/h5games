...
events.prototype.insertAction = function (action, x, y, callback) {
  if (core.status.event.id == null) {
    ...
    if (core.isset(callback))
      core.status.event.data.callback = callback;
  }
}
...
events.prototype.afterPushBox = function () {
  ...
  var noBoxLeft = function () {
    ...
    for (var i = 0; i < core.status.thisMap.blocks.length; i++) {
      ...
      if (core.isset(block.event) && block.event.id == 'box')
        return false;
    }
    return true;
  }
...
events.prototype.clickSL = function (x, y) {
  ...
  if (y >= 1 && y <= 4) {
    ...
    if (x >= 5 && x <= 7)
      core.doSL(5 * page + 1, core.status.event.id);
    if (x >= 9 && x <= 11)
      core.doSL(5 * page + 2, core.status.event.id);
  }
  if (y >= 7 && y <= 10) {
    if (x >= 1 && x <= 3)
      core.doSL(5 * page + 3, core.status.event.id);
    if (x >= 5 && x <= 7)
      core.doSL(5 * page + 4, core.status.event.id);
    if (x >= 9 && x <= 11)
      core.doSL(5 * page + 5, core.status.event.id);
  }
}
